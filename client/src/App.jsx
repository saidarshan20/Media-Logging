...
export default function App() {
    ...
    return logs
      ...
      .sort((a, b) => {
        ...
        if (a.status === "WATCHING") {
          const progA =
            a.episode && a.total_episodes ? a.episode / a.total_episodes : 0;
          const progB =
            b.episode && b.total_episodes ? b.episode / b.total_episodes : 0;
  ...
  const availableYears = [
    ...new Set(logs.map((l) => l.release_year).filter(Boolean)),
  ]
    .sort()
    .reverse();
    ...
    if (finalStatus !== "DROPPED" && total > 0) {
      if (ep === total) finalStatus = "COMPLETED";
        ...
        if (finalStatus === "COMPLETED") {
          ...
          const isStatusChange = originalLog
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="bg-surface border border-stone-700 p-8 rounded-2xl shadow-2xl w-full max-w-sm text-center">
          ...
          <p className="text-stone-400 text-sm mb-6">
            Enter your secret code to access logs.
          </p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            ...
            <button
              type="submit"
              className="w-full bg-stone-100 hover:bg-white text-stone-900 font-bold py-3 rounded-lg transition-colors"
            >
              Unlock
            </button>
          </form>
      ...
      <header className="mb-6 md:mb-8">
            ...
            <img
              ...
              onError={(e) => {
                ...
                e.target.parentElement.className +=
                  " text-2xl font-bold text-stone-500";
              }}
            />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="text-xs text-stone-500 hover:text-red-400 border border-stone-800 px-2 py-1 rounded"
              >
                Logout
              </button>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs md:text-sm text-stone-400 mt-1">
              <div className="relative">
                ...
                >
                  ...
                  {getDateRangeLabel()} <ChevronDown className="w-3 h-3" />
                </button>
                {isDateMenuOpen && (
                  <>
                    <div
                    ...
                    <div className="absolute top-full left-0 mt-1 w-32 bg-stone-800 border border-stone-700 rounded-lg shadow-xl overflow-hidden z-20">
                      <button
                        ...
                        className="w-full text-left px-3 py-2 hover:bg-stone-700 text-stone-300 text-xs"
                      >
                        All Time
                      </button>
                      <button
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between">
          <div className="flex bg-surface p-1 rounded-lg border border-stone-700 w-full md:w-auto">
            <button
              ...
              className={`flex-1 md:flex-none px-6 py-2 md:py-1.5 text-sm font-medium rounded-md transition-all ${
                viewMode === "ALL"
            ...
            <button
              ...
              className={`flex-1 md:flex-none px-6 py-2 md:py-1.5 text-sm font-medium rounded-md transition-all ${
                viewMode === "ONGOING"
                  ? "bg-emerald-900/50 text-emerald-400 border border-emerald-900/50 shadow"
                  : "text-stone-400 hover:text-stone-200"
              }`}
            >
              Ongoing
        ...
        {viewMode === "ALL" && (
          <div className="flex flex-wrap gap-2 mt-4 items-center">
            ...
            <select
              className="bg-surface border border-stone-700 text-stone-300 text-xs rounded px-2 py-1.5 outline-none"
              value={filters.year}
            ...
            {(filters.year !== "ALL" ||
              ...
              <button
                ...
                className="text-xs text-stone-500 hover:text-white ml-2 underline"
              >
                Clear
              </button>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredLogs.map((log) => {
          ...
          return (
            <article
              key={log.id}
              className="bg-surface rounded-xl border border-stone-700/50 p-5 shadow-lg group hover:border-stone-600 transition-all relative overflow-hidden"
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-[10px] flex items-center gap-1 ${
                      STATUS_CONFIG[log.status || "COMPLETED"].color
                    }`}
                  >
                    <StatusIcon className="w-3 h-3" />{" "}
                    {STATUS_CONFIG[log.status || "COMPLETED"].label}
                  </span>
                </div>
              ...
              <h3
                className={`text-lg font-bold leading-tight mb-2 truncate pr-4 ${ratingColor}`}
              >
                {log.title}
              </h3>
              ...
              {log.type === "SERIES" && (log.season || log.episode) && (
                <div className="flex items-center justify-between text-xs text-stone-300 bg-stone-900/50 rounded px-2 py-1.5 mb-3 border border-stone-800">
                  ...
                  {log.total_episodes && log.status === "WATCHING" && (
                    <span className="text-emerald-500 font-mono">
                      {Math.round((log.episode / log.total_episodes) * 100)}%
                    </span>
                  )}
                </div>
              )}
